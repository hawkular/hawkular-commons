...
import org.hawkular.commons.doc.*
...
def handlersDir = new File(classesDir, "org/hawkular/inventory/handlers")
...
json["info"] = [version: inventoryVersion, title: "Hawkular Inventory"]
json["basePath"] = "/hawkular/inventory"
json["tags"] = []
json["paths"] = [:]
json["definitions"] = [:]
        ...
        if (genericType.actualTypeArguments.length > 0) {
            ...
            for (def j = 0; j < genericType.actualTypeArguments.length; j++) {
                if (genericType.actualTypeArguments[j] instanceof ParameterizedType) {
                    ...
                    if (genericType.actualTypeArguments[j] instanceof TypeVariable) {
                        fieldType += "Object"
                    } else {
...
try {
    // [lponce] This class was thought for multiple handler classes (as it happens in Alerting)
    // [lponce] Inventory only have a single class but I won't remove the loop yet
    File[] handlers = handlersDir.listFiles()
    for (def i = 0; i < handlers.length; i++) {
        if (handlers[i].name.endsWith("Handlers.class")) {
            def className = "org.hawkular.inventory.handlers." + handlers[i].name.substring(0, handlers[i].name.length() - 6)
            ...
            if (clazz.isAnnotationPresent(DocModel.class)) {
                ...
                for (def i = 0; i < fields.length; i++) {
                    if (fields[i].isAnnotationPresent(DocModelProperty.class)) {
                        ...
                        json["definitions"][className]["properties"][fieldName]["required"] = docModelProperty.required()
                        json["definitions"][className]["properties"][fieldName]["position"] = docModelProperty.position()
